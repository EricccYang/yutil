...
template <typename Queue>
BenchmarkResult run_yutil_bench(std::string name,
...
BenchmarkResult run_rigtorp_mpmc(std::size_t producer_count,
                                 std::size_t consumer_count,
                                 std::size_t items_per_producer,
                                 std::size_t capacity) {
  const std::size_t total_items = producer_count * items_per_producer;
  rigtorp::MPMCQueue<std::uint64_t> queue(capacity);
  std::atomic<std::size_t> done_producers{0};
  std::atomic<std::uint64_t> sum{0};
  ...
  const std::uint64_t expected_sum =
      (static_cast<std::uint64_t>(total_items) * static_cast<std::uint64_t>(total_items + 1)) / 2;
  if (sum.load(std::memory_order_relaxed) != expected_sum) {
    std::cerr << "unexpected sum for rigtorp::MPMCQueue\n";
  }

  const auto elapsed_ns =
      static_cast<double>(std::chrono::duration_cast<std::chrono::nanoseconds>(end - start).count());
  return BenchmarkResult{"rigtorp::MPMCQueue",
                         elapsed_ns / 1e6,
                         elapsed_ns / static_cast<double>(total_items),
                         static_cast<double>(total_items) / elapsed_ns * 1e3};
}
...
int main() {
  ...
  const auto baseline =
      run_blocking_baseline<yutil::queue::MutexDequeMpmcQueueBaseline<std::uint64_t>>(
          "MutexDequeMpmcQueueBaseline(V2)", kProducerCount, kConsumerCount, kItemsPerProducer);
  const auto v1 = run_yutil_bench<yutil::queue::BoundedAtomicMpmcQueueV1<std::uint64_t>>(
      "BoundedAtomicMpmcQueueV1", kProducerCount, kConsumerCount, kItemsPerProducer, kCapacity);
  const auto v2 = run_yutil_bench<yutil::queue::BoundedAtomicMpmcQueueV2<std::uint64_t>>(
      "BoundedAtomicMpmcQueueV2", kProducerCount, kConsumerCount, kItemsPerProducer, kCapacity);
  const auto v3 = run_yutil_bench<yutil::queue::BoundedAtomicMpmcQueueV3<std::uint64_t>>(
      "BoundedAtomicMpmcQueueV3", kProducerCount, kConsumerCount, kItemsPerProducer, kCapacity);
  const auto rigtorp = run_rigtorp_mpmc(kProducerCount, kConsumerCount, kItemsPerProducer, kCapacity);

  std::cout << "Bounded atomic MPMC iteration benchmark (" << kProducerCount << "P/" << kConsumerCount
  ...
  print_result(rigtorp, baseline.ns_per_op);
  return 0;
}
