...
void test_mpmc_correctness() {
  ...
  constexpr std::size_t kItemsPerProducer = 25000;
  ...
  assert(total_count.load(std::memory_order_relaxed) == kTotalItems);
  assert(total_sum.load(std::memory_order_relaxed) == expected_sum);
}

}  // namespace

int main() {
  test_basic_fifo<yutil::queue::MutexDequeMpmcQueueV1<int>>();
  test_basic_fifo<yutil::queue::MutexDequeMpmcQueueV2<int>>();
  test_basic_fifo<yutil::queue::MutexDequeMpmcQueueV3<int>>();

  test_close_behavior<yutil::queue::MutexDequeMpmcQueueV1<int>>();
  test_close_behavior<yutil::queue::MutexDequeMpmcQueueV2<int>>();
  test_close_behavior<yutil::queue::MutexDequeMpmcQueueV3<int>>();

  test_mpmc_correctness<yutil::queue::MutexDequeMpmcQueueV1<std::uint64_t>>();
  test_mpmc_correctness<yutil::queue::MutexDequeMpmcQueueV2<std::uint64_t>>();
  test_mpmc_correctness<yutil::queue::MutexDequeMpmcQueueV3<std::uint64_t>>();

  std::cout << "mpmc mutex queue version tests passed\n";
  return 0;
}
